...
public class Wii : MonoBehaviour
{
    ...
    public static bool GetButton(int remote, string s)
    {
        ...
        switch (s)
	    {
			...
			case "WIIUPROZR":
				return currentStates[remote].expButton13;
			case "WIIUPROL3":
				return currentStates[remote].expFloat5==1f;
			case "WIIUPROR3":
				return currentStates[remote].expFloat6==1f;
    ...
    public static bool GetButtonDown(int remote, string s)
    {
	    ...
	    switch (s)
	    {
			...
			case "WIIUPROZR":
				return !previousStates[remote].expButton13 && currentStates[remote].expButton13;
			case "WIIUPROL3":
				return previousStates[remote].expFloat5!=1f && currentStates[remote].expFloat5==1f;
			case "WIIUPROR3":
				return previousStates[remote].expFloat6!=1f && currentStates[remote].expFloat6==1f;
	...
	public static Vector2 GetAnalogStick(int remote)
    {
		...
		switch(currentStates[remote].expType)
		{
			...
			case (char)7:
				return GetAnalogStick(remote,"WIIUPROLEFT");
			default:
				return Vector2.zero;
    ...
    public static Vector2 GetAnalogStick(int remote, string s)
    {
		...
		if(s == "CLASSICRIGHT" || s == "WIIUPRORIGHT")
		{
		...
		switch(s)
    	{
    	    ...
    	    case "TURNTABLE":
    	    	...
    	    	return new Vector2(tStickX,tStickY);
			case "WIIUPROLEFT":
				return GetWiiUProAnalogStick(remote,"LEFT");
			case "WIIUPRORIGHT":
				return GetWiiUProAnalogStick(remote,"RIGHT");
			default:
    	    	return Vector2.zero;
    	}
    }
	...
	public static bool  GetWiiUProButton(int remote, string s)
	{
		if(remote<0 || remote>(max-1))
		{
			Debug.LogWarning("invalid remote number");
		...
		if(currentStates[remote].expType==7)
		{
			switch(s)
			{	
			case "A":
	...
	public static bool  GetWiiUProButtonDown(int remote, string s)
	{
		if(remote<0 || remote>(max-1))
		{
			Debug.LogWarning("invalid remote number");
	...
	public static bool  GetWiiUProButtonUp(int remote, string s)
	{
		if(remote<0 || remote>(max-1))
		{
			Debug.LogWarning("invalid remote number");
	...
	//get WiiUPro stick
	// LY  - 1200
	private static float uProStickNormalizer = 1200f;
	public static Vector2 GetWiiUProAnalogStick(int remote, string s)
	{
		if(remote<0 || remote>(max-1))
		{
			Debug.LogWarning("invalid remote number");
		...
		//just so the custom inspector can work when it's not playing.
		if (Time.time == 0) {
			currentStates[remote] = getStates(remote);
		}

		if(currentStates[remote].expType!=7 || float.IsNaN(currentStates[remote].expFloat1) || float.IsNaN(currentStates[remote].expFloat3))
			return Vector2.zero;

		if (wiiUProLeftCalibs == null) {
			wiiUProLeftCalibs = new Vector2[max];
			wiiUProRightCalibs= new Vector2[max];
		}

		if (wiiUProLeftCalibs [remote] == Vector2.zero) {
			CalibrateWiiUPro(remote);
		}

		//Debug.Log (new Vector2((currentStates[remote].expFloat1-wiiUProLeftCalibs[remote].x)/uProStickNormalizer, (currentStates[remote].expFloat2-wiiUProLeftCalibs[remote].y)/uProStickNormalizer));

		switch(s)
		{
		case "LEFT":
			return new Vector2((currentStates[remote].expFloat1-wiiUProLeftCalibs[remote].x)/uProStickNormalizer, (currentStates[remote].expFloat2-wiiUProLeftCalibs[remote].y)/uProStickNormalizer);
		case "RIGHT":
			return new Vector2((currentStates[remote].expFloat3-wiiUProRightCalibs[remote].x)/uProStickNormalizer, (currentStates[remote].expFloat4-wiiUProRightCalibs[remote].y)/uProStickNormalizer);
		default:
			print("Unsupported Analog Stick Type " + s);
			return Vector2.zero;
		}
	...
	private static Vector2[] wiiUProLeftCalibs;
	private static Vector2[] wiiUProRightCalibs;
	public static void CalibrateWiiUPro(int remote)
	{
		wiiUProLeftCalibs [remote] = new Vector2(currentStates[remote].expFloat1, currentStates[remote].expFloat2);
		wiiUProRightCalibs [remote] = new Vector2(currentStates[remote].expFloat3, currentStates[remote].expFloat4);
	}
	...
	public static event DiscoveryError OnDiscoveryFailed;
	public static event WiimoteFound OnWiimoteDiscovered;
	public static event WiimoteDropped OnWiimoteDisconnected;
	
    //private const string pluginName = "WiiBuddy";
	#if  UNITY_64 || UNITY_EDITOR_64
	private const string pluginName = "WiiBuddy64";
	#else
	private const string pluginName = "WiiBuddy32";
	#endif
   ...
   // IRData               6     
   	...
   	public struct State
    {
		...
		#if  UNITY_64 && UNITY_STANDALONE_WIN
    	...
    	if(searching || Application.isEditor)//(because the extension could also cause new ones to be discovered at any time.)
    	{
    		...
    		if(status>99)
    		{
    			...
    			else
    			{
					...
					//new way
					if(OnWiimoteDiscovered != null)
					{
						OnWiimoteDiscovered(i);
					}
    	...
    	for(int i=0;i<highestNumberedPlayer;i++)
    	{
    		if(wiiRemotesActive[i] && !isVirtual[i])
    		{
	  	 		...
	  	 		if(currentStates[i].active == false && previousStates[i].active == true)
    			{
					...
					//the new way
					if(OnWiimoteDisconnected != null)
					{
						OnWiimoteDisconnected(i);
					}
					//the old way
